...
const apiEndpoints: ApiEndpoint[] = [
  {
    ...
    description: 'Search for products with advanced filtering, pagination, and sorting',
    parameters: [
      { name: 'q', type: 'string', required: false, description: 'Search query string' },
      { name: 'supermarket', type: 'string', required: false, description: 'Filter by supermarket' },
      ...
      { name: 'in_stock', type: 'boolean', required: false, description: 'Filter by stock status' },
      { name: 'on_special', type: 'boolean', required: false, description: 'Filter by special offers' },
    ...
    example: {
      ...
      response: { products: [], total: 0, page: 1, limit: 20, has_next: false, has_prev: false }
    }
  },
  {
    method: 'GET',
    ...
    parameters: [
      { name: 'barcode', type: 'string', required: true, description: 'The product barcode' }
    ],
    example: {
  ...
  },
  {
    ...
    parameters: [
      ...
      { name: 'gtin', type: 'string', required: false, description: 'Product GTIN' },
      { name: 'sku', type: 'string', required: false, description: 'Product SKU' },
      { name: 'supermarket', type: 'string', required: false, description: 'Supermarket name' },
      { name: 'days', type: 'number', required: false, description: 'Limit to last N days' }
    ],
    example: {
  ...
  },
  {
    ...
    parameters: [
      ...
      { name: 'strategy', type: 'string', required: false, description: 'Recommendation strategy' }
    ],
    example: {
      request: { product_id: 123, limit: 5 },
      response: { product: {}, recommendations: [], strategy: 'similar', total_recommendations: 5 }
    }
  }
];
...
const ApiDocumentation: React.FC = () => {
    ...
    try {
      ...
      switch (selectedEndpoint.path) {
        case '/products/search':
          result = await apiClient.searchProducts(requestParams as SearchParams);
          break;
  ...
  return (
    <div className="api-documentation">
      <h1>OzGroceries API Documentation</h1>
      <p className="description">
        Explore and test the OzGroceries API endpoints for Australian grocery product data.
      </p>
